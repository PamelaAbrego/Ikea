    ...
    try:
        with connection.cursor() as cursor:
            ...
            if opcion == 1:
                ...
                connection.commit()
                print(
                    "Se ha borrado definitivamente el registro de la tabla Países: "
                    + datosReg
                )
                getAllPaises()
...
def getAllCiudades():
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT ciudades.idCiudades, ciudades.nombre, paises.idPaises, paises.nombre as nombrePais from ikea.ciudades inner join ikea.paises on ciudades.idPaises = paises.idPaises;"
            cursor.execute(sql)
            result = cursor.fetchall()
    finally:
        table = PrettyTable()
        table.field_names = ["idCiudades", "nombre", "idPaises", "nombrePais"]
...
def deleteCiudad():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            registro = result[0]
            datosReg = (
                str(registro["idCiudades"])
                + " "
                + registro["nombre"]
                + " "
                + str(registro["idPaises"])
            )
            print(
                "idCiudad: "
                + str(registro["idCiudades"])
