...
def insertUsuarioBD(
    ...
    try:
        with connection.cursor() as cursor:
            sql = f"""insert into ikea.usuarios (nombre, apellido, segundoApellido, telefono, idioma, correoElectronico, contrasenna, direccion)
            values ('{nombre}','{apellido}', '{segundoApellido}', '{telefono}', '{idioma}', '{correoElectronico}', '{contrasenna}', '{direccion}');"""
            cursor.execute(sql)
            connection.commit()
    finally:
        pass
...
def updateUsuarioBD(
    ...
    try:
        with connection.cursor() as cursor:
            sql = f"""UPDATE ikea.usuarios SET nombre = '{nombre}',apellido = '{apellido}', segundoApellido ='{segundoApellido}',
